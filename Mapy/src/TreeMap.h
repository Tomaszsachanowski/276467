  ...
  class Node
  {
    ...
    void remove(Node* temp)
    {
        ...
        if(temp->left == nullptr && temp->right == nullptr)
            {
                if (temp->parent == nullptr) root = nullptr;
                else
        ...
        else if (temp->left != nullptr && temp->right == nullptr)
            {
                if (temp->parent == nullptr)
                {
                    ...
                    else if (temp == temp->parent->right)
                    {
                        ...
                        throw std::out_of_range("Something totally wrong");
            }
        // 1 right kid
        else if (temp->left == nullptr && temp->right != nullptr)
            {
...
public:
  ...
  using pointer = const typename TreeMap::value_type*;
    ...
    {
        if (node == nullptr) throw std::out_of_range("increasing end()");
    ...
    ConstIterator& operator--()
    {
        if (*this == map->begin() || map->size <= 0) throw std::out_of_range("decreasing begin or empty map");
    ...
    {
		if (*this == map->end()) throw std::out_of_range("reference out of range");
    ...
    {
		if (*this == map->end()) throw std::out_of_range("pointer out of range");
